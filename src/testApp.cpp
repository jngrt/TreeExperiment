...
void testApp::draw() {
    ...
    ofPoint treeScale = treeModel.getScale();
    float treeRotAngle = treeModel.getRotationAngle(0);
    ofPoint treeRotAxis = treeModel.getRotationAxis(0);
    ofSetColor(100,100,254);
    ofPushStyle();
    shapesMat.begin();
    //ofRotate(treeRotAngle, treeRotAxis.x, treeRotAxis.y, treeRotAxis.z);
    treeShape->transformGL();
    ofScale(treeScale.x, treeScale.y, treeScale.z);
    treeModel.getMesh(0).drawFaces();
    treeShape->restoreTramsformGL();
    shapesMat.end();
    ofPopStyle();
